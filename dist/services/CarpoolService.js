...
export class CarpoolService {
    ...
    create(cardGroupId, data) {
        ...
        // 检查卡组是否存在
        const cardGroup = db.prepare('SELECT id, deadline FROM card_groups WHERE id = ?').get(cardGroupId);
        if (!cardGroup) {
    ...
    `);
        ...
        const carpool = this.getById(result.lastInsertRowid);
     ...
     */
    getByCardGroup(cardGroupId, status) {
        // 检查卡组是否存在
    ...
    `;
        ...
        // 计算每个拼车的进度
        const carpoolsWithProgress = carpools.map((c) => {
        ...
        `)
            ...
            for (const assignment of activeAssignments) {
                try {
                    const selectedMembers = JSON.parse(assignment.selected_members || '[]');
            ...
            let creator = null;
            if (c.creator_id) {
                creator = db.prepare('SELECT * FROM users WHERE id = ?').get(c.creator_id);
            }
    ...
    getWithAssignments(id) {
      ...
      `)
        ...
        if (carpool.creator_id) {
            creator = db.prepare('SELECT * FROM users WHERE id = ?').get(carpool.creator_id);
        }
        // 如果没有系统用户，使用creator_name
     ...
     * 新逻辑：所有成员都被且只被一个用户选择
     */
    checkAndLockIfComplete(carpoolId) {
     ...
     */
    updateStatus(carpoolId, data, ownerId) {
        const carpool = this.getById(carpoolId);
        // 预留权限功能：如果提供了ownerId，验证车主权限（暂时不强制）
        if (ownerId) {
            const cardGroup = db.prepare('SELECT owner_id FROM card_groups WHERE id = ?').get(carpool.card_group_id);
...
//# sourceMappingURL=CarpoolService.js.map