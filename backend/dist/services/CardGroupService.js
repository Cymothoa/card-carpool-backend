...
export class CardGroupService {
    ...
    `);
        ...
        const cardGroup = this.getById(result.lastInsertRowid);
     ...
     */
    getById(id) {
        const cardGroup = db.prepare('SELECT * FROM card_groups WHERE id = ?').get(id);
        if (!cardGroup) {
    ...
    getWithMembers(id) {
        ...
        const members = db
            ...
            .all(id);
        const owner = db.prepare('SELECT * FROM users WHERE id = ?').get(cardGroup.owner_id);
        return {
            ...cardGroup,
            members,
            owner,
...
//# sourceMappingURL=CardGroupService.js.map