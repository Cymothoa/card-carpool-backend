...
import { CreateMemberRequest, BulkCreateMembersRequest } from '../models/Member.js';
 ...
 * 添加成员到卡组
 */
router.post('/:groupId/members', (req: Request, res: Response, next: NextFunction) => {
  try {
 ...
 * 批量添加成员（cortis预设）
 */
router.post('/:groupId/members/bulk', (req: Request, res: Response, next: NextFunction) => {
  try {
    const groupId = parseInt(req.params.groupId, 10);
    if (isNaN(groupId)) {
      return res.status(400).json({ error: 'Invalid card group ID' });
    }
    const data: BulkCreateMembersRequest = req.body;
 ...
 * 获取卡组的成员列表
 */
router.get('/:groupId/members', (req: Request, res: Response, next: NextFunction) => {
  try {
    const groupId = parseInt(req.params.groupId, 10);
    if (isNaN(groupId)) {
      return res.status(400).json({ error: 'Invalid card group ID' });
