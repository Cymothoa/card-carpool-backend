...
import {
  ...
  AssignmentResult,
} from '../models/MemberAssignment.js';
...
export class MemberAssignmentService {
  ...
  create(carpoolId: number, data: CreateAssignmentRequest): AssignmentResult {
    ...
    // 检查拼车是否存在
    const carpool = db.prepare('SELECT * FROM carpools WHERE id = ?').get(carpoolId) as any;
    if (!carpool) {
    ...
    // 检查卡组是否超过截止时间
    const cardGroup = db.prepare('SELECT deadline FROM card_groups WHERE id = ?').get(carpool.card_group_id) as { deadline: string | null };
    ...
    const existingAssignments = db
      .prepare(`
        ...
        AND username = ?
      `)
      .all(carpoolId, trimmedUsername) as Array<{ id: number; username: string }>;
    ...
    const transaction = db.transaction(() => {
      ...
      for (const existing of assignmentsWithMembers) {
        ...
        if (intersection.length > 0) {
          ...
          if (newCardCount <= existingCardCount) {
            // 新用户总卡数不够，不能挤掉
            throw new Error(
              `无法挤掉：您选择的卡数（${newCardCount}）必须大于现有用户（${existingCardCount}）`
            );
          }
          ...
          logger.info(
            `User ${trimmedUsername} (user_id: ${user_id || 'null'}) squeezed out user ${existing.username} (user_id: ${existing.user_id || 'null'}) for members [${intersection.join(',')}] in carpool ${carpoolId}`
          );
        }
      }
      ...
      const newAssignment = this.getById(result.lastInsertRowid as number);
    ...
    logger.info(
      `Member assignment created: ${result.assignment.id} for members [${selected_members.join(',')}] in carpool ${carpoolId}`
    );
   ...
   */
  getByCarpool(carpoolId: number, activeOnly: boolean = true): MemberAssignment[] {
    let query = `
    ...
    const assignments = db.prepare(query).all(carpoolId) as any[];
  ...
  getById(id: number): MemberAssignment {
    const assignment = db
      ...
      .get(id) as MemberAssignment | undefined;
    ...
    const parsed = {
      ...assignment,
      selected_members: JSON.parse((assignment.selected_members as any) || '[]'),
    };
  ...
  delete(id: number): void {
    ...
    // 检查拼车是否已锁定
    const carpool = db.prepare('SELECT status FROM carpools WHERE id = ?').get(assignment.carpool_id) as {
      status: string;
    };
    if (carpool.status === 'locked' || carpool.status === 'driven' || carpool.status === 'cancelled') {
