...
export class MemberService {
  ...
  create(cardGroupId: number, data: CreateMemberRequest): Member {
    ...
    const member = this.getById(result.lastInsertRowid as number);
  ...
  bulkCreate(cardGroupId: number, data: BulkCreateMembersRequest): Member[] {
    let membersToCreate: CreateMemberRequest[] = [];
    ...
    const transaction = db.transaction((members: CreateMemberRequest[]) => {
      for (const memberData of members) {
        try {
          ...
          const result = insertStmt.run(
            cardGroupId,
            memberData.member_number.trim(),
            memberData.member_name?.trim() || null
          );
          const member = this.getById(result.lastInsertRowid as number);
          created.push(member);
        } catch (error) {
