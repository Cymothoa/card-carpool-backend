...
import { CreateAssignmentRequest } from '../models/MemberAssignment.js';
 ...
 * 分配成员（加入拼车或挤车）
 */
router.post('/carpools/:carpoolId/assignments', (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const data: CreateAssignmentRequest = req.body;
 ...
 * 获取拼车的成员分配列表
 */
router.get('/carpools/:carpoolId/assignments', (req: Request, res: Response, next: NextFunction) => {
  try {
    const carpoolId = parseInt(req.params.carpoolId, 10);
    if (isNaN(carpoolId)) {
      return res.status(400).json({ error: 'Invalid carpool ID' });
 ...
 */
router.delete(
  '/carpools/:carpoolId/assignments/:assignmentId',
